...
logger = logging.getLogger("Unmanic.Plugin.encoder_video_hevc_libx265")
...
class PluginStreamMapper(StreamMapper):
    ...
    def custom_stream_mapping(self, stream_info: dict, stream_id: int):
        ...
        if settings.get_setting('advanced'):
            stream_encoding = ['-c:v:{}'.format(stream_id), 'libx265']
            stream_encoding += settings.get_setting('custom_options').split()
        else:
            stream_encoding = [
                '-c:v:{}'.format(stream_id), 'libx265',
